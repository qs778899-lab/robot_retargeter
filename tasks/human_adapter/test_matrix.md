...
- BVH 大腿 orientation 门禁：`left_thigh/right_thigh` 对应 robot `knee_link`，其 quaternion 不能只由 `hip -> knee` 或 `knee -> hip` 单根大腿向量决定。PNS/v3 都应从几何构造 knee frame：`+Z = knee -> hip`，`+X = pelvis facing` 投影到垂直于 `+Z` 的平面，`+Y` 由右手系得到。修改该逻辑必须同时重跑 PNS/v3 数值门禁与可视化回归。
...
- foot floating/contact 门禁：
  - BVH keypoints pkl 必须写入 robot config 中的 foot/toe `contact_names`，且这些 contact 点必须存在于 `keypoint_names`。
  - `contact_states` 必须基于最终保存的 keypoints 坐标计算，不能使用高度对齐前的旧 contact positions。
  - PNS `pick_up2274_chr00` 必须记录 foot/toe contact active counts、active contact Z min/mean/max、sampled foot/toe Z min/mean/max。
  - 若 active contact 很少但保存后的足端高度已经接近地面，必须优先检查 contact 判定顺序和 ground alignment，而不是调 IK cost。
