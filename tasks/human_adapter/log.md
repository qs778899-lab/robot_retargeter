...
## 2026-06-26：PNS 腿部异常定位与数值修复

- 继续排查 `pick_up2274_chr00`：
  - keypoints 全局 Y 下会出现左右脚交换，但以 hip/pelvis lateral 为基准的局部左右脚间距始终为正：`local lateral sep min/mean/max = 0.1663/0.3485/0.6280m`，局部交叉帧数为 `0`。
  - base robot 输出局部交叉也为 `0`，但腿部 rotation error 大，且髋/踝位置误差偏高：`left_hip mean/max = 0.0804/0.1879m`、`right_hip mean/max = 0.0893/0.1995m`。
  - 加 root/hips orientation cost 未解决该问题，说明主因不是单纯 root yaw 没跟随。
- 隔离实验：
  - 临时关闭 PNS 腿部 orientation cost（`left/right_hip`、`left/right_thigh`、`left/right_calf`）后，局部交叉仍为 `0`。
  - 髋/踝位置误差显著下降：
    - `left_hip mean/max = 0.0063/0.0491m`
    - `left_calf mean/max = 0.0040/0.0936m`
    - `right_hip mean/max = 0.0057/0.0479m`
    - `right_calf mean/max = 0.0029/0.0437m`
  - 膝盖局部左右间距更稳定：base `knee_sep min=0.0468m`，关闭腿部 orientation 后 `knee_sep min=0.1760m`。
- 修复：
  - `scripts/human_replay.py` 改为按格式生成 `ik_orientation_cost_scales`：
    - v3：保持腿部 orientation cost，只关闭手臂 orientation cost（v3 已可视化确认正常）。
    - PNS：关闭手臂和腿部 orientation cost，让 IK 优先满足髋、膝、踝、腕的位置目标。
  - 标准 PNS 链路重新生成 `output_data/robot_motion/pick_up2274_chr00_g1.csv`，不再依赖临时 YAML。
- 标准 PNS 输出验证：
  - pkl scale 确认：`left_hip/left_thigh/left_calf/right_hip/right_thigh/right_calf = 0.0`。
  - sampled local cross：`0 / 204`。
  - sampled foot Z：`[-0.1021, 0.0419, 0.0739]`。
  - sampled knee local sep：`[0.1760, 0.2781, 0.4316]`。
  - sampled ankle local sep：`[0.1683, 0.3486, 0.6350]`。
- v3 回归：
  - 标准 v3 keypoints/retarget 已重跑，输出仍为 `output_data/robot_motion/Hand-Reaching-62-pangyiming-v3-1-pangyiming-v3_g1.csv`，shape `(208, 36)`。
  - v3 pkl 不关闭腿部 orientation cost，仅保留手臂 orientation override。
- 当前状态：
  - PNS 数值门禁通过。
  - 仍需用户可视化确认 `pick_up2274_chr00` 腿部观感是否满足要求。
